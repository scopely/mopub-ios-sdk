...
-(void)requestRewardedVideoWithCustomEventInfo:(NSDictionary *)info
{
    ...
    if(zoneId && [zoneId isKindOfClass:[NSString class]]) {
        ...
        NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorInvalidCustomEvent userInfo:@{NSLocalizedDescriptionKey: @"invalid zone id"}];
    ...
    if(adUnitId && [adUnitId isKindOfClass:[NSString class]]) {
        ...
        NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorInvalidAdUnitID userInfo:@{NSLocalizedDescriptionKey: @"invalid ad unit id"}];
        if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToLoadAdForCustomEvent:error:)]) {
            [self.delegate rewardedVideoDidFailToLoadAdForCustomEvent:self error:error];
        }
        return;
    ...
    if(![mopubManager isInitialized]) {
        ...
        if(zoneIds && appId) {
            ...
            NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorInvalidCustomEvent userInfo:@{NSLocalizedDescriptionKey: @"invalid app id or zone ids"}];
            if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToLoadAdForCustomEvent:error:)]) {
                [self.delegate rewardedVideoDidFailToLoadAdForCustomEvent:self error:error];
            }
            return;
    ...
    if(!doingInit && [AppOnboardSDK nextPresentationIdToShowInZoneId:self.zoneId] == nil) {
        if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToLoadAdForCustomEvent:error:)]) {
            NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorNoAdsAvailable userInfo:@{NSLocalizedDescriptionKey: @"No fill"}];
            [self.delegate rewardedVideoDidFailToLoadAdForCustomEvent:self error:error];
        }
        return;
    }
...
-(void)presentationPreparationFailedNotification:(NSNotification *)notif
{
    ...
    if(!error || ![error isKindOfClass:[NSError class]]) {
        ...
        error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorUnknown userInfo:@{NSLocalizedDescriptionKey: @"unknown error"}];
    }
...
-(void)presentRewardedVideoFromViewController:(UIViewController *)viewController
{
    ...
    if(self.isReady) {
        if([AppOnboardSDK shouldShowPresentationForZoneId:self.zoneId]) {
            if(![AppOnboardSDK showPresentationForZoneId:self.zoneId delegate:self]) {
                if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToPlayForCustomEvent:error:)]) {
                    NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorAdUnitWarmingUp userInfo:@{NSLocalizedDescriptionKey: @"Unable to show presentation at this time"}];
            ...
            if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToLoadAdForCustomEvent:error:)]) {
                NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorAdAlreadyPlayed userInfo:@{NSLocalizedDescriptionKey: @"Unable to show presentation at this time"}];
                [self.delegate rewardedVideoDidFailToLoadAdForCustomEvent:self error:error];
            }
        }
    }
        ...
        // no ad is available
        if([self.delegate respondsToSelector:@selector(rewardedVideoDidFailToLoadAdForCustomEvent:error:)]) {
            NSError *error = [NSError errorWithDomain:MoPubRewardedVideoAdsSDKDomain code:MPRewardedVideoAdErrorNoAdsAvailable userInfo:@{NSLocalizedDescriptionKey: @"No presentation available at this time"}];
            [self.delegate rewardedVideoDidFailToLoadAdForCustomEvent:self error:error];
        }
    }
